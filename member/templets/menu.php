    <?php
        ...
        if(!empty($cfg_ml->M_ID))
        {
            $channelInfos = array();
            //sh：只选定video
            $dsql->Execute('addmod',"SELECT id,nid,typename,useraddcon,usermancon,issend,issystem,usertype,isshow FROM `#@__channeltype` WHERE id=$channelid");	
    ...
    <div id="mcpsub">
      ...
      <div id="menuBody">
        ...
        <?php
      	...
      	{
      	?>
        <h2 class="menuTitle" onclick="menuShow('menuFirst')" id="menuFirst_t"><b></b>视频管理</h2>
        <ul id="menuFirst">
        <?php 
        if($channelInfos['video']['issend']==1 && $channelInfos['video']['isshow']==1)
        {//是否启用视频投稿
        ?>
          <li class="articles"><a href="../member/content_list.php?channelid=17" title="已发布视频"><b></b>视频</a><a href="../member/video_add.php" class="act" title="发表新文章">投稿</a></li>
        <?php
        }
        ?>
        <?php
        	var_dump($channelInfos);
        //是否启用文章投稿
        ...
        <!--  
				...
				{
				?>
        <h2 class="menuTitle" onclick="menuShow('menuSec')" id="menuSec_t"><b></b>视频收藏</h2>
        <ul id="menuSec">
